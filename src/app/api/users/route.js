...
export async function POST(request) {
  try {
    ...
    const { nom, prenom, email, type_utilisateur, service, poste, type_contrat, date_debut_contrat, date_fin_contrat, date_entree_mairie } = body;
    ...
    const result = await db.execute({
      sql: `
        INSERT INTO users (nom, prenom, email, mot_de_passe, type_utilisateur, service, poste, mot_de_passe_temporaire, type_contrat, date_debut_contrat, date_fin_contrat, date_entree_mairie)
        VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?)
      `,
      args: [nom, prenom, email, hashedPassword, type_utilisateur, service || null, poste || null, type_contrat || 'CDI', date_debut_contrat || null, date_fin_contrat || null, date_entree_mairie || null]
    });
